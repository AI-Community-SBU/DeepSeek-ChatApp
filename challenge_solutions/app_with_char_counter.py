...
with st.sidebar:
    ...
    # Add character count display area in the sidebar
...
# Update character count in real-time via callback
def on_input_change():
    # Get the current input length
    char_count = len(st.session_state.current_input)
    
    # Display the character count with warning if it's getting long
    if char_count > 500:
        char_count_placeholder.markdown(f"**Characters:** {char_count} ⚠️")
    else:
        char_count_placeholder.markdown(f"**Characters:** {char_count}")
...
if st.sidebar.button("Clear Chat History"):
    st.session_state.messages = []
    char_count_placeholder.markdown("**Characters:** 0")
